...
export function getResourceFilesInfoData(): InfoPanelData {
  return {
    ...
    text:
      ...
      'A node considered to be in the lowest fog layer typically utilises the least resources, thus the LPDS_T2 consists of 12 CPU cores and 24 GB RAMs' +
...
export function getApplicationInfoData(): InfoPanelData {
  return {
    ...
    text:
      'The task size attribute tells the highest amount of unprocessed data that can be packaged in one compute task to be executed by virtual machines.' +
      'A daemon service chechs regularly the repository for unprocessed data based on the frequency value.' +
      'The number of instruction defines the maximum value which one task can represent.' +
      'The threshold determines how many unprocessed task can be hold in the actual application'
        +'the further tasks are forwarded according to the strategy of the application.' +
      'The can join allows IoT devices to send unprocessed data directly into the application.' +
      'The VM flavor is used for executing the compute tasks, which can be specified in the instance tag.' +
      'You have the following possibilites in case of the strategy value:'+
      ...
      'Finally, the Runtime-aware strategy ranks the available parent nodes, and all neighbour nodes (from its own layer) by network latency' 
        +'and by the ratio of the available CPU capacity and the total CPU capacity. The algorithm picks the node with the highest rank.' +
...
export function getStationInfoData(): InfoPanelData {
  return {
    title: 'Stations',
    text:
    'We can configure the life time of the device (starttime, stoptime), the number of sensors it has (sensor),' 
      + 'the size of the generated data (filesize) and the generation and sending frequency.' +
    'The network  settings  of  the  local  repository  are defined by the maxinbw, maxoutbw and diskbw tags,'
        + 'and the size of the repository is determined by the reposize field.' +
    'The radius defines a range where a set of IoT devices are randomly placed,' 
      +'the number of the devices located at the range is defined by the number attribute.'+
    'You have the following possibilites in case of the strategy value:'+
    ...
    'The runtime-aware strategy takes into account the actual load of the available nodes.' +
